...
class MasterSortOrderTestCase(unittest.TestCase):
    ...
    def test_master_form_shows_next_sort_order_readonly_on_create(self):
        client = self.app.test_client()
        client.post("/auth/login", data={"username": "admin", "password": "admin"})
        response = client.get("/master/items")
        html = response.data.decode()
        self.assertEqual(response.status_code, 200)
        self.assertIn("Urutan dibuat otomatis berdasarkan data terakhir.", html)
